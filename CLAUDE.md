...
- **Tablets/wide (`isWide`): unchanged.** `NowPlayingPane` (side pane in the `SupportingPaneScaffold`) + `NowPlayingSheet` (modal, opened by tapping the bottom-bar `MiniPlayer`). The tablet's adaptive pane split is known-flaky (`// This doesn't work :3` on the pane expansion).
