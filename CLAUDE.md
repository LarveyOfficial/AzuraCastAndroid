...
- `NowPlayingData.staticDataMap` — `Map<Pair<url, shortCode>, StationJSON>` cache of the AzuraCast now-playing metadata for every known station; `staticData` is the currently-playing station's entry. (Field names keep the `static` prefix for historical reasons; the data is the live per-station now-playing response.)
...
- `GET /api/nowplaying/{shortCode}` → `StationJSON`. The primary metadata source — AzuraCast's [per-station now-playing endpoint](https://www.azuracast.com/docs/developers/now-playing-data/). Fetched via `AzuraCastRepository.getNowPlayingData` (seeds the cache and refreshes the playing item's metadata). Replaced the older cached `/api/nowplaying_static/{shortCode}.json` static file. Every API request is cache-busted with a `_ts` timestamp param (see `CacheBustingInterceptor`).
...
2. calls `setMediaMetadata(..., reset=true)` → fetches the now-playing metadata via the repository, then `applyNowPlayingMetadata` builds a `MediaItem` with full `MediaMetadata` (title/artist/album/art/duration), `replaceMediaItem(0, ...)`, then `prepare()` + `play()` — all on the main thread via the `@ApplicationScope` scope.

Note the metadata quirks in `NowPlayingData.applyNowPlayingMetadata` ([classes/models/NowPlayingData.kt](app/src/main/java/com/larvey/azuracastplayer/classes/models/NowPlayingData.kt)): Android Auto reads `subtitle` as artist and `description` as album, so those fields are set redundantly alongside the "correct" ones — keep both when editing. `staticData.value` is set to the freshly-fetched `data` (and the same `data` is written into `staticDataMap`), so the currently-playing snapshot reflects the current fetch immediately. (It previously read `Map.put()`'s return — the *old* entry, not the new one — which lagged the progress UI one fetch behind; that was a bug from misreading `put()`'s return value, now fixed.) Separately, `applyNowPlayingMetadata` calls `replaceMediaItem`, which re-fires the service's `onMediaMetadataChanged`; because the follow-up fetch returns identical metadata, Media3 de-dupes and the chain stops after one extra fetch per song change (a known redundant round-trip, not an infinite loop).
...
Two very different surfaces by form factor:
- **Phones (`!isWide`): an expanding card, NOT a sheet.** The mini player and the full Now Playing screen are ONE bottom-anchored card that grows continuously from the mini bar into the full screen (swipe up / tap to expand; drag down / back / Stop to collapse). Hosted as an overlay in `MainActivity` (a top-drawn sibling of the pane scaffold), gated `!isWide`.
  - [ExpandingPlayerState.kt](app/src/main/java/com/larvey/azuracastplayer/ui/nowplaying/ExpandingPlayerState.kt): a `translationY` `Animatable` is the single source of truth (0 = expanded, `collapsedY` = collapsed); `expansion` (0..1) is *derived*. Drag `snapTo`s it 1:1 (`UNDISPATCHED`), clamped `[0, collapsedY]`; settle by 5dp / 150px-vel.
  - [ExpandingNowPlayer.kt](app/src/main/java/com/larvey/azuracastplayer/ui/nowplaying/ExpandingNowPlayer.kt): the growing card. Two nested `Modifier.layout`s — OUTER grows width (side margins 12dp→0) + height (mini→full); INNER lays content at full height, bottom-aligned, revealed by the clip (never stretched). One shared solid album background (`albumColors().container`) with the **mesh gradient fading in over it** (solid→mesh morph), then the full player fading in, then the bare `MiniPlayerContent` fading out. Reads `expansion` only in `graphicsLayer`/`layout`/`derivedStateOf` (no per-frame recomposition). `PredictiveBackHandler` drives collapse; Stop animates collapse→slide-away→stop; the nav bar slides down by its height as it opens.
  - [NowPlayingContent.kt](app/src/main/java/com/larvey/azuracastplayer/ui/nowplaying/NowPlayingContent.kt): the full-player body (top bar + album art + controls + queue `NavHost`), rendered with `drawBackground = false` inside the card. `NowPlayingBackground` is the shared backdrop composable.
- **Tablets/wide (`isWide`): a docking side pane, no modal.** `NowPlayingPane` (side pane in the `SupportingPaneScaffold`); the old `NowPlayingSheet` modal was removed. The bottom-bar `MiniPlayer` (tablet-only) shows only when the Now Playing pane is *not* up — i.e. while a Discover station's detail occupies the pane; tapping it closes that detail and docks the Now Playing pane in its place. **`miniPlayerVisible` is derived as the exact complement of the pane's own visibility flags** (`currentMediaItem != null && (discoveryViewingStation || hidingDiscoverDetail)`), **not** from the `SupportingPaneScaffold` navigator's `PaneAdaptedValue` — that value is known-flaky (`// This doesn't work :3`) and, because a fold/unfold **recreates the activity** (MainActivity has no `android:configChanges`, so all `remember` state resets), it would desync and render the mini and the pane at once.

Inside the full player (both forms): a `SharedTransitionLayout` + nested `NavHost` with `"nowPlaying"` (big album art + song/artist) and `"queue"` (`NowPlayingHistory`: up-next + song history). Album art is a **shared element** animating between the two.
...
Non-`main` branches are protected by a **ruleset** (require PR, signed commits, no force-push/deletion) plus **required status checks**. [PR Checks](.github/workflows/pr-check.yml) run as **four separate checks** — `Unit Tests`, `Lint`, `Debug Build`, `Release Build` — each its own job (so failures are attributable at a glance). The `Debug Build` job uploads the debug APK as a run artifact (`app-debug-apk`) and posts/updates a sticky PR comment linking to it. `Release Build` builds `assembleRelease` unsigned (no signing env on PRs) but still runs R8/minify + resource shrinking, so missing ProGuard keep rules fail the PR. **The ruleset's required status checks must list all four job names** (`Unit Tests`, `Lint`, `Debug Build`, `Release Build`) — not the old single `build`.

Which checks actually run is decided by a lightweight always-on **`changes`** job that classifies the PR's changed *paths* (not their contents) into `code` (app sources, gradle/build config, ProGuard) vs `tests` (`app/src/test`/`androidTest`): **Unit Tests** runs on a `code` **or** `tests` change; **Lint/Debug Build/Release Build** run only on a `code` change (a test-only PR skips them); a docs-only PR (`*.md`, screenshots, release-please metadata, workflow yaml) runs **nothing**. Draft PRs also run nothing. The gating is per-job `if:` (not a workflow-level `paths:` filter) **on purpose** — a *skipped* required check reports as "skipped", which branch protection treats as passing, so the PR stays mergeable; a `paths:` filter would stop the check from ever reporting and block the PR forever. The `changes` job fails **open** (runs everything) if it can't read the diff.
